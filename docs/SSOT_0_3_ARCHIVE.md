...
> Historický archiv. Tento dokument není aktuálním zadáním; platí [SSOT.md](SSOT.md).
...
| Finanční statistiky | `/financial-stats` | `from=YYYY-MM-DD`, `to=YYYY-MM-DD` (ověřeno živě 11. 9. 2026) | JSON snapshot, pouze diagnostika |
...
Počáteční datum se čte z databázového nastavení `sync.start_date`, které uživatel mění v Nastavení přes kalendář „Načítat pomocná data od“. Chybějící nebo prázdná hodnota se při otevření databáze inicializuje na 2026-01-01; vyplněná hodnota zůstává zachována. Konec je vždy aktuální den. Finanční data ani předchozí pokrytí rozsah nerozšiřují. Datum musí být platné, neprázdné a nejvýše dnešní. Změna platí pro příští úplný import; nemaže finanční zdroje ani existující párování. Období se čte po nepřekrývajících se blocích, standardně 7 kalendářních dní, včetně obou krajů.

Rezervace se navíc filtrují lokálně podle překryvu pobytu s celým zvoleným obdobím: příjezd <= konec a odjezd >= začátek. Při chybějícím termínu v seznamu se načte detail; chybějící či obrácený interval i po doplnění ukončí import bez publikace. Rezervace mimo období se nezařadí do aktivních pomocných dat a nestahují se jejich vazby. Vybrané rezervace si zachovají všechny související doklady i mimo období. API může vrátit širší seznam, takže lokální filtr negarantuje omezení přenesených seznamových dat. Checkpoint obsahuje verzi pravidel výběru `STAY-OVERLAP-6-INVOICE-ROW-1`; starší verze nebo změněné nastavení vyvolají nový běh.

Položky faktur jsou výskyty řádků náležející konkrétní faktuře. BetterHotel může použít stejné API ID pro běžnou položku a odečet zálohy `Deposit`. Interní ID je hash verze `INVOICE-ROW-1`, ID faktury, původního ID položky, hashe normalizovaného obsahu a pořadí mezi obsahově identickými výskyty. Původní ID zůstává v `source_item_id`, původní odpověď v raw snapshotu. Zachovají se všechny výskyty a znaménka; změna pořadí řádků nemění množinu identit. Kontrola shody reprezentací celé faktury zůstává přísná. Soukromé podklady skutečných konfliktů se ukládají do `diagnostics/conflicts`, běžný log obsahuje jen identifikátor diagnostiky, typ entity, endpoint a názvy rozdílných polí.
...
3. Jednou za celé období načíst všechny reservation stránky s oběma expand, provést lokální výběr a u vybraných rezervací načíst detail. Poté zpracovat úplné vztahy invoice a jejich potřebné detaily, bill a bill items včetně detailů a security deposits. Seznam rezervací se neopakuje v každém bloku dokladů. Cache stejné reprezentace má klíč (context_id,generation_id,resource_type,external_id,projection_kind,request_shape_hash). LIST nikdy nenahradí požadovaný DETAIL pouhou shodou ID.
...
5. Po každém bloku dokladů, dokončení seznamu a výběru rezervací a každých 25 dokončených rezervacích uložit snapshots, observations, kandidátní entity/hrany a checkpoint s fází běhu do STAGING generace krátkou transakcí. Poslední neúplná dávka se také uloží. **Nezměnit publikovaný graf ani jeho aktivní množiny.** GUI může ukázat zvlášť označený průběžný náhled běhu; párování, hledání aktuálních helperů a export aktuálního grafu jej nesmějí zaměnit za publikovaná data.
...
| sync.start_date | 2026-01-01 při inicializaci databázové hodnoty | Povinné uživatelské datum nejvýše dnešní; určuje přesný začátek importu |
